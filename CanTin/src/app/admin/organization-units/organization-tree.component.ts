...
export class OrganizationTreeComponent extends AppComponentBase implements OnInit {
    ...
    nodeSelect(event) {
        this.ouSelected.emit(<IBasicOrganizationUnitInfo>{
            ...
            parentLabelId: event.node.data.parentLabelId
        });
    }
    ...
    private getTreeDataFromServer(): void {
        ...
        this._organizationUnitService.getOrganizationUnits().subscribe((result: ListResultDtoOfOrganizationUnitDto) => {
            ...
            this.data_DVXL = result;
            console.log(result)
            if(self.orgId > 0){
                self.treeData = self._arrayToTreeConverterService.createTree(result.items,
                    'parentId',
                    'id',
                    8,
                    'children',
        ...
        let items = [
            {
                ...
                command: (event) => {
                    ...
                    this.createOrEditOrganizationUnitModal.show({
                        ...
                        parentLabelId: this.selectedOu.data.parentLabelId
                    });
                }
            },
            {
    ...
    addUnit(parentId?: number): void {
        if(this.appSession.organizationUnitId > 0){
            parentId = this.appSession.organizationUnitId;
        }
        this.createOrEditOrganizationUnitModal.show({
            parentId: parentId
        });
