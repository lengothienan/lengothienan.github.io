import { AfterViewChecked, Component, ElementRef, EventEmitter, Injector, Output, Renderer2, ViewChild } from '@angular/core';
...
export class CreateOrEditUserModalComponent extends AppComponentBase {
    ...
    show(userId?: number): void {
        ...
        this._userService.getUserForEdit(userId).subscribe(userResult => {
            ...
            if(that.appSession.organizationUnitId > 0){
                userResult.roles.splice(userResult.roles.findIndex(x => x.roleName == 'Admin'), 1);
                userResult.roles.splice(userResult.roles.findIndex(x => x.roleDisplayName == 'Giám đốc'), 1);
                userResult.roles.splice(userResult.roles.findIndex(x => x.roleDisplayName == 'Phó Giám đốc'), 1);
