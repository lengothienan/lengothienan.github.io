...
import { DxDataGridComponent, DxPopupComponent, DxFormComponent, DxTreeViewComponent, DxChartComponent, DxPieChartComponent, DxTooltipComponent } from 'devextreme-angular';
import { isNullOrUndefined } from 'util';
import { delay } from 'q';
import { DRReportServiceProxy, DRReportServiceServiceProxy, DRLookUpServiceProxy, DRViewerServiceProxy, DRFilterServiceProxy, DrDynamicFieldServiceProxy, FParameter, DRColumnServiceProxy, DRChartServiceProxy, DRServiceDto } from '@shared/service-proxies/service-proxies';
...
export class ReportEditComponent extends AppComponentBase implements OnInit, OnChanges {
    ...
    ngOnChanges() {
        ...
        this.activeRouter.params.subscribe(param => {
            self.reportService.getIdByCode(self.reportCode != undefined ? self.reportCode : param.code).subscribe(async (data: any) => {
                ...
                self.http.get(AppConsts.fileServerUrl + '/api/v1/Attachment/get/uploadfile/objectid?object_id=' + self.currentReport + "&code=EDITOR_TEMPLATE").subscribe(res => {
                    if (res['result']['isSucceeded']) {
                        if (res['result']['data'][0] != undefined) {
                            self.linkDownLoadTemplate = res['result']['data'][0].downloadPath;
                        }
                    }
                })
                await self.reportService.getById(self.currentReport).subscribe((res: any) => {
        ...
        if (self.displayType == 0) {
            ...
            self.dataSourceTest3 = {
                ...
                update: async (key, values) => {
                    ...
                    const promise = new Promise((resolve, reject) => {
                        self.viewerService.editData(self.currentReport, key.Id, key).subscribe(res => {
                            resolve(res.data);
                        });
                    });
                        ...
                        if (res[i].serviceId != null) {
                            ...
                            col['lookup'] = {
                                dataSource: new CustomStore({
                                    key: service.colValue,
                                    byKey: async (key) => {
                                        let data: any = await self.getDRDataSource(service.code);
                                        return data.find(e => e.id == key)[0];
                                    },
                                    load: async (loadOptions) => {
                                        return await self.getDRDataSource(service.code);
                                    }
                                }),
    ...
    async getDRDataSource(serviceCode) {
        const self = this;
        let promise = new Promise((resolve, reject) => {
            self.serviceService.executeServiceByCode(serviceCode, null).subscribe(_res => {
                resolve(_res.data);
            }, err => {
                reject(err);
            });
