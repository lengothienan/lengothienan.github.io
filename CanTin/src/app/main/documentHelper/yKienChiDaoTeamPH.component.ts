...
@Component({
    selector: 'yKienChiDaoTeamPH',
    templateUrl: './yKienChiDaoTeamPH.component.html',
    animations: [appModuleAnimation()]
})
//ý kiến chỉ đạo cho vb đến từ đội
export class YKienChiDaoTeamPHComponent extends AppComponentBase implements OnInit{
    ...
    getInitial(){
        ...
        if(this.capNhatDto.listDocs.length == 1){
            this._documentHandlingAppService.get_DVXL_ForDocument_Dept_Phoi_Hop(this.capNhatDto.listDocs[0].documentHandlingDetailId).subscribe((res) => {
                this.old_DVXL = res;
    ...
    save(){
        this.setListDVXL();
        if(!this.capNhatDto.listDVXLs || this.capNhatDto.listDVXLs.filter(x => x.unitId >= 0 && x.userId > 0 && x.typeHandling==1).length == 0){
            this.message.warn('Vui lòng chọn cán bộ chỉ đạo xử lý');
            return;
        }
        this.disabled = true;
        this._documentAppService.multipleCapNhatChiDaoDoiPH(this.appSession.selfOrganizationUnitId, this.capNhatDto)
        .pipe(finalize(()=>{
