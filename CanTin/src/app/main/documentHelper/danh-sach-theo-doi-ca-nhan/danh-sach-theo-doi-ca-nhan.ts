...
import { HistoryUploadsServiceProxy, DocumentServiceProxy, Idoc_starsServiceProxy, CreateOrEditIdoc_starDto, DocumentHandlingDetailDto, DocumentHandlingsServiceProxy, DocumentHandlingDetailsServiceProxy, UserServiceProxy, DynamicGridHelperServiceProxy, DynamicFieldsServiceProxy, DirectorOpinionDto, ListDVXL, ApproveDocumentDto, LabelsServiceProxy, CapNhatDanhGiaDto } from '@shared/service-proxies/service-proxies';
...
@Component({
    templateUrl: './danh-sach-theo-doi-ca-nhan.html',
    styleUrls: ['./danh-sach-theo-doi-ca-nhan.less'],
    animations: [appModuleAnimation()]
})
export class VanBanTheoDoiCaNhanComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit() {
        ...
        this._activatedRoute.params.subscribe(params => {
            ...
            this._labelAppService.getNameOfLabelByUserId(this.labelId).subscribe(res => {
                this.header = res;
            });

            this._activatedRoute.queryParams.subscribe(pr =>{

                let tungay = pr['tungay'];
                let denngay = pr['denngay'];
                let type = pr['type'];
                let outdate = params['outdate']=='outdate'?true:false;
    
                if(tungay != undefined && denngay != undefined && type != undefined){
                    this.getVanbanDxTable(this.labelId, outdate, type, tungay, denngay);
                }else{
                    this.getVanbanDxTable(this.labelId, outdate);
                }
            });
    ...
    transfer_head_department_cellTemplate() {
        this.ultility.selectedRows = [];
        this.getSelectedRowKeys();
    ...
    getVanbanDxTable(labelId: number, outdate = false, type = 1, tungay = "2000-01-01", denngay= '') {
        this._documentAppService.getAllDocumentToPersonNotHandled(labelId, outdate, type, tungay, denngay).subscribe(result => {
            console.log(result);
    ...
    cap_nhat_kqxl(data){
      if(data.TypeProcess==1){
        this._documentAppService.getVanBanCapNhatXuLy(data.Id).subscribe((p:any)=>{
        ...
        this._documentAppService.getVanBanCapNhatXuLyPH(data.DocumentHandlingDetailId,data.Id).subscribe((p:any)=>{ 
    ...
    updateRating = (): void => {
        ...
        if (this.status==1 && isNullOrUndefined(this.rate)){
            ...
            if(this.ratingData.TypeProcess==1){
                this._documentAppService.updateDanhGia(data).subscribe(()=>{
                    this.getVanbanDxTable(this.labelId);
                    this.ratePopupVisible=false;
                }) 
            }else{
                this._documentAppService.updateDanhGiaPH(data).subscribe(()=>{  
                    this.getVanbanDxTable(this.labelId);
                    this.ratePopupVisible=false;
                })
            }
    ...
    onExporting(e){
        ...
        e.component.columnOption('bcdd', 'visible', true);
    ...
    onExported(e){  
        ...
        e.component.columnOption('bcdd', 'visible', false);
