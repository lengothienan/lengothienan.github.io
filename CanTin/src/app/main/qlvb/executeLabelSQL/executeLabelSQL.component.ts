...
export class ExecuteLabelSQLComponent extends AppComponentBase implements OnInit {
    ...
    // getMonthCurrent=(new Date).getMonth() + 1;
    data = [];
    ...
    numberDVCellValue(rowData){
        return rowData.IncommingNumber;
    }

    // numberCellValueFilter(filterValue, selectedFilterOperation){

    //     const column = this as any;

    //     const filterExpression = [
    //         [column.dataField[0], "=", filterValue],
    //         "or",
    //         [column.dataField[1], "contains", filterValue]
    //     ];
    ...
    getVanbanDxTable(labelId: number, outdate = false) {
        ...
        if (labelId == 85)
            ...
            this._dynamicGridServiceProxy.getAllDataAndColumnConfigByLabelId(labelId, outdate).subscribe(result => {
                ...
                this.data = result.getDataAndColumnConfig.listData;
                ...
                //Khóa filter theo tháng
                // this.initialData = this.data.filter(x => x.month == this.getMonthCurrent);
                this.initialData = this.data;
                that.totalCount = that.initialData.length;
               
                that.gridContainer.instance.repaint();
                // console.log(this.dataBookSelectBox.instance.option("value"));
                // console.log(this.monthSelectBox.instance.option("value"));
            });
    ...
    showListHistory(event: any) {
        ...
        // });
        this.dataDisplay = [];
        let row = this.initialData.find(element => element.Id == event);
    ...
    async getListDocumentHandlingDetailByDocumentHandlingId(handlingId: any)
    { 
        const that = this;
        let promise = new Promise(function (resolve, reject) {
            that._documentHandlingAppService.getListDocumentHandlingDetailByDocumentHandlingId(handlingId).subscribe(res => {
                resolve(res);
            });
        });
        return promise;
    ...
    async approveDocument() {
        ...
        let hasDVXL = false;
        let listDVXL: any = await this.getListDocumentHandlingDetailByDocumentHandlingId(this.selectedRowsData[0].HandlingId);    
        if (listDVXL != null && listDVXL.length > 0) {
            for (const element of listDVXL) {
                if (element.TypeHandling == 1) {
                    hasDVXL = true;
                }
            }
        }
        if (!hasDVXL) {
            this.message.warn('Vui lòng phân đơn vị chủ trì xử lý trước khi duyệt');
            return;
        }
    ...
    async approve_Multiple_Func() {
        ...
        for (var i = 0, len = this.selectedRowsData.length; i < len; i++) {
            let hasDVXL_VB = false;
            let listDVXL: any = await this.getListDocumentHandlingDetailByDocumentHandlingId(this.selectedRowsData[i].HandlingId);      
            debugger
            if (listDVXL != null && listDVXL.length > 0) {
                for (const element of listDVXL) {
                    if (element.TypeHandling == 1) {
        ...
        if (!hasDVXL) {
            this.message.warn('Vui lòng phân đơn vị chủ trì xử lý cho các văn bản đã chọn trước khi duyệt');
            return;
        }
        //return;
        this.spinnerService.show();
    ...
    //             self.selectedRowsFile = self.selectedRowsFile.concat(self.dataDisplay);
    ...
    onValueChanged_book(e: any) {
        ...
        //     this.initialData = dataSource.filter(x => (x.BookType == e.value));
        // } else if (e.value == 0 && month != 0) {
        //     this.initialData = dataSource.filter(x => (x.month == month));
        // } else {
        //     this.initialData = dataSource.filter(x => (x.BookType == e.value && x.month == month));
        // }
        let dataSource = [...this.data];
        if (e.value == 0) {
            this.initialData = dataSource;
    ...
    onValueChanged_month(e: any) {
        ...
        //     this.initialData = dataSource.filter(x => x.BookType == e.book);
        // } else {
        //     this.initialData = dataSource.filter(x => (x.month == e.value && x.BookType == book));
        // }
        // this.totalCount = this.initialData.length;
    }
    ...
    //in nhiều phiếu trình
    printPhieuTrinh() {
        if (this.selectedRowsData.length > 0) {
            this.selectedRowsData.forEach(row => {
                $.ajax({
                    url: this.printUrl + row.Id,
                    method: 'POST',
                    data: {
                        documentId: row.Id,
                        roleId: this.appSession.roleId
        ...
        } else this.notify.warn("Chưa chọn văn bản in phiếu trình!");
