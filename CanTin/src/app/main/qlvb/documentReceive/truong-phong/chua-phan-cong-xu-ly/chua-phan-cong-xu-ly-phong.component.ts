...
@Component({
    templateUrl: './chua-phan-cong-xu-ly-phong.component.html',
    styleUrls: ['./chua-phan-cong-xu-ly-phong.component.less'],
    animations: [appModuleAnimation()]
})
export class ChuaPhanCongXuLyPhongComponent extends AppComponentBase implements OnInit {
    ...
    processRecommend = '' //ý kiến xử lý ô phân công
    orgId: number;
    constructor(
        injector: Injector,
        ...
        this.orgId = this.appSession.organizationUnitId;
    ...
    getTeamInOrg(){
        ...
        this._documentAppService.getListDepartment(this.appSession.organizationUnitId).subscribe(res =>{
            ...
            this.loaiDV[1].dataSource = this.data_department;
        });
    }

    getUserInDept(){
        this._oDocsServiceProxy.getUserInRoleByOrg(this.appSession.organizationUnitId, 'PP').subscribe(res => {
            this.chi_huy = res;
            for(var i = 0, len = this.chi_huy.length; i < len; i++){
                this.chi_huy[i]["unitId"] = this.orgId;
    ...
    save(){
        ...
        this._documentAppService.transferDocumentToCaptain(this.selectedRowsData[0]["Id"], this.selectedRowsData[0]["HandlingId"], this.dataRowDetail.processRecommend, this.listDVXL_selected).subscribe(()=>{
    ...
    edit(e: any){
        this.router.navigate(['/app/main/qlvb/so-den-theo-phong/' + e.Id]);
    }
    ...
    setListDVXL(){
        ...
        if(this.listDVXL_selected.findIndex(x => x.typeHandling == 1) == -1){
            this.message.warn('Vui lòng chọn Phó phòng hoặc Đội chủ trì');
            return;
        }      
    }

    onCheckBoxChanged(e, cell, tabIndex: number)
    {
        if(tabIndex == 0){
            console.log(cell)
            let index = this.chi_huy.findIndex(x => x.userId == cell.data.userId);
            console.log(index)
            ...
            switch(cell.column.dataField){
                case 'mainHandling':
                    ...
                    if(this.previousUserMainHandlingId >= 0){               
                        let temp = this.chi_huy.findIndex(x => x.userId == this.previousUserMainHandlingId);
                        this.chi_huy[temp]["mainHandling"] = false;
                    }
        
                    if(this.chi_huy[index]["coHandling"] == true){
                        this.chi_huy[index]["coHandling"] = false;
                    }
        
                    this.chi_huy[index]["mainHandling"] = e.value;   
    
                    //giữ id của đơn vị đang nắm chủ trì
                    this.previousUserMainHandlingId = cell.data.id;
                    break;
                case 'coHandling':
                    if(this.chi_huy[index]["mainHandling"] == true){
                        this.chi_huy[index]["mainHandling"] = false;
                    }
        
                    this.chi_huy[index]["coHandling"] = e.value;
            }
        }
        else if(tabIndex == 1){
