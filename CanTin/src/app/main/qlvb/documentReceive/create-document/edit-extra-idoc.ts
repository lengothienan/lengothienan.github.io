...
@Component({
    selector: 'editExtraIDoc',
    templateUrl: './edit-extra-idoc.html',
...
export class EditExtraIDocComponent extends AppComponentBase implements OnInit{
    ...
    //get nơi gửi
    // getPublishOrg(){
    //     this._publishOrgAppService.getAllPublishOrg().subscribe((res)=>{ this.data_publisher_Initial = res;console.log(this.data_publisher_Initial) })
    // }
    getPublishOrg(){
                ...
                this._documentHandlingAppService.get_DVXL_ForDocument(result.id, result.unitId).subscribe((res) => {
                    ...
                    this.old_DVXL.forEach((ele) => {
                        ...
                        if(ele.TypeHandling == 1){
                            ...
                            this.data_department[index]["disabledPHXL"] = true;
    ...
    getIncommingNumber(soVB: number) {
        this._documentAppService.getNextIncommingNumber(soVB).subscribe(res => {
            this.documentData.incommingNumber = res;
        });
    }
    ...
    save(): void {
        ...
        // this.setListDVXL();
        this.json = JSON.stringify(this.newSelectedDVXL);
        if(result.isValid){
            this.saving = true;
            ...
            this.documentData.incommingDate = moment(this.incommingDateVal).utc(true);
            ...
            this.documentData.approvingRecommended = this.documentData.approvingRecommended;
            this.documentData.subApprovingRecommended = this.documentData.subApprovingRecommended;
            this._documentAppService.extraUpdateDocument(this.json, this.documentData)
                .pipe(finalize(() => { this.saving = false;}))
                .subscribe(() => {
                    this.notify.success('Sửa bổ sung thành công');
                    this.router.navigate(['/app/main/qlvb/executeLabelNewSQL/85']);
                });
        }
    }
    ...
    saveHandlingDetail(docId, handlingId){
        this.data_department.forEach(receiver => {
            ...
            if(receiver.mainHandling == true || receiver.coHandling == true){
                ...
                record.processingRecommended = this.documentData.approvingRecommended;
                if(receiver.mainHandling == true){
