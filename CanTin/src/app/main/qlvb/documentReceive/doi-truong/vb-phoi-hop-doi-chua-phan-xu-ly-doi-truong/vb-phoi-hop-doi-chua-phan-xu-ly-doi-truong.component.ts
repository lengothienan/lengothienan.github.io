...
import $ from 'jquery';
import { YKienChiDaoTeamPHComponent } from '@app/main/documentHelper/yKienChiDaoTeamPH.component';
import * as moment from 'moment';

@Component({
  selector: 'app-vb-phoi-hop-doi-chua-phan-xu-ly-doi-truong',
  templateUrl: './vb-phoi-hop-doi-chua-phan-xu-ly-doi-truong.component.html',
  styleUrls: ['./vb-phoi-hop-doi-chua-phan-xu-ly-doi-truong.component.css'],
  animations: [appModuleAnimation()]
}) 
export class VbPhoiHopDoiChuaPhanXuLyDoiTruongComponent extends AppComponentBase implements OnInit {
  ...
  @Input() actionID: string;
    ...
    @ViewChild('yKienChiDaoTeam', { static: false}) yKienChiDao: YKienChiDaoTeamPHComponent;
    totalCount: number = 0;
    saving = false;
    rawSql: string;
    user_ID: any;
    ...
    async cap_nhat_chi_dao(e){    
        ...
        let arr = [];
        let dl = '';
        self.selectedRowsData = this.gridContainer.instance.getSelectedRowsData();
        ...
        for(var i = 0, len = this.selectedRowsData.length; i < len; i++){
           ...
           debugger
            ...
            console.log(self.selectedRowsData[i])
            await self._documentAppService.seenDocument(self.selectedRowsData[i].DocumentHandlingDetailId)         
            .toPromise().then(res=>{
    ...
    edit(e: any){
        this.router.navigate(['/app/main/qlvb/sua-vb-den-doi-phoi-hop/' + e.data.Id]);
    }
    ...
    search() {
        ...
        this._documentAppService.getAllDocumentHaveDirectedToTeamPhoiHop(this.documentSearchData.fromDate, this.documentSearchData.toDate, this.appSession.selfOrganizationUnitId).subscribe(res => {
