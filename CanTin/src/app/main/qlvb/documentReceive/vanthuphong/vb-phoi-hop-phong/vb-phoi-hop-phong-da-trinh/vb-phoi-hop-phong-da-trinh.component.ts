...
import { GetDocInputForSearchDto,DocumentForViewDto,GetUserInRoleDto, ODocsServiceProxy,HistoryUploadsServiceProxy, DocumentServiceProxy, Idoc_starsServiceProxy, CreateOrEditIdoc_starDto, DocumentHandlingDetailDto, DocumentHandlingsServiceProxy, DocumentHandlingDetailsServiceProxy, UserServiceProxy, DynamicGridHelperServiceProxy, DynamicFieldsServiceProxy, DirectorOpinionDto, ListDVXL, ApproveDocumentDto, CapNhatInputDto, EditDocumentHandlingDetailDto } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'app-vb-phoi-hop-phong-da-trinh',
  templateUrl: './vb-phoi-hop-phong-da-trinh.component.html',
  styleUrls: ['./vb-phoi-hop-phong-da-trinh.component.css'],
  animations: [appModuleAnimation()]
})
export class VbPhoiHopPhongDaTrinhComponent  extends AppComponentBase implements OnInit {
  ...
  @ViewChild('documentForm', {static: true}) documentForm: DxFormComponent;
    ...
    popup_EditPhanCong:boolean=false;
    constructor(
        injector: Injector,
        ...
        private _documentHandlingDetail:DocumentHandlingDetailsServiceProxy,
    ...
    view(e: any){
        this.router.navigate(['/app/main/qlvb/them-vb-phong-phoi-hop/view/' + e.Id+'/'+e.documentHandlingDetailId]);
    }
    ...
    popupClose_EditPhanCong(){
        this.popup_EditPhanCong=false;
    }
    ...
    data_department = [];
    edit(e: any){
        this.popup_EditPhanCong=true;
        this.getTeamInOrg();
        this.getUserInOrg();
        this.getUserInOrg_PhoiHop(e);

    }
    onCheckBoxChangedEdit(e, cell)
    { 
    ...
    getUserInOrg_PhoiHop(d) {
        ...
        this.iddochandlingdetail=iddochandlingdetail;
        this.documentId=Id;
        if(Id != null){
            this.documentId = Number.parseInt(Id);
            this.iddochandlingdetail=Number.parseInt(iddochandlingdetail);
            this._documentAppService.getDocumentForViewPhoiHop(iddochandlingdetail,Id).subscribe((result) => {
                ...
                this._documentHandlingAppService.get_DVXL_ForDocument_Dept_Phoi_Hop(iddochandlingdetail).subscribe((res) => {
                    ...
                    this.TruongPhongPhoPhong = res.filter(x => x['ParentHandlingId'] == 0 || x['ParentHandlingId'] == null)[0];
                    ...
                    if(this.TruongPhongPhoPhong['PTPProcessingDate']==null){
                        this.ppDate =""
                    }
                    ...
                    if(this.TruongPhongPhoPhong['TPProcessingDate']==null){
                        this.tpDate =""
                    }
                    doi.forEach((ele) => {
                        let index = this.data_department.findIndex(x => x.id == ele.OrganizationId);
                        if (index != -1){
                            this.data_department[index]["isResolved"]=ele.isResolved;
                            if(ele.TypeHandling == 1){
                                if(ele.isResolved == true){
                                    this.daxuly=true;
                                    this.data_department[index]["isResolved"]=true;
                                }
    ...
    save(): void {
        ...
        // if(result.isValid){
            this.setListDVXL();
        if(!this.listDVXL_selected || this.listDVXL_selected.filter(x => x.unitId > 0 && x.typeHandling==1).length == 0){
            this.popup_EditPhanCong=false;
            let result = this.message.warn("Vui lòng chọn Đội chủ trì", "Cảnh báo");
           ...
           this.capnhatDocHandlingDetail.ptpUserId=this.phoPhongVal;
           this.capnhatDocHandlingDetail.ptpProcessingDate=this.ppDate;
           this.capnhatDocHandlingDetail.ptpProcessingRecommended= this.ppComment;
           this.capnhatDocHandlingDetail.tpUserId= this.truongPhongVal;
           this.capnhatDocHandlingDetail.tpProcessingDate=this.tpDate;
           this.capnhatDocHandlingDetail.tpProcessingRecommended= this.tpComment;
           this.capnhatDocHandlingDetail.listDVXLs=this.listDVXL_selected;
           this._documentAppService._CapNhatChiDaoPhongPhoiHop(this.capnhatDocHandlingDetail).subscribe(res=>{
    ...
    search() {
        ...
        this._documentAppService.getListVBTuCATPDaCoChiDaoPhoiHop(this.documentSearchData.fromDate, this.documentSearchData.toDate, this.appSession.organizationUnitId).subscribe(res => {
