...
export class EditDocumentDenPhongExtraComponent extends AppComponentBase implements OnInit{
    ...
    orgLevelDisabled:boolean=false;
    currentUser:any;
    constructor(
        injector: Injector,
    ...
    ngOnInit(){
        ...
        if(Id != null){
            ...
            this._documentAppService.getDocumentEditByDocumentId(this.documentId).subscribe((result) => {
                ...
                this.orgLevelDisabled = true;

                self._documentHandlingAppService.get_DVXL_ForDocument_Dept(result.id).subscribe((res) => {


                    self.old_DVXL = res;
                    let doi = res.filter(x => x['OrganizationId'] !== 0 && x['HandlingType'] == 4);
                    self.phoPhongObj = res.filter(x => x['TypeHandling'] == 0 && x['OrganizationId'] == 0 && x['HandlingType'] == 4)[0];
                    if(self.phoPhongObj){
                    ...
                    self.truongPhongObj = res.filter(x => x['TypeHandling'] == 1 && x['OrganizationId'] == 0 && x['HandlingType'] == 4)[0];
                    ...
                    doi.forEach((ele) => {
                        ...
                        if (index != -1){
                            this.data_department[index]["isResolved"]=ele.isResolved!=null?ele.isResolved:false;
                ...
                this._documentAppService.checkDaPhanCBCS(this.documentData.id).subscribe(res=>{
                    let index = this.data_department.findIndex(p => p.mainHandling == true);
                    if(res){
                        this.daxuly=false;
                    }else{
                        this.daxuly=true;
    ...
    //kiểm tra xem đã có ý kiến chỉ đạo của trưởng or phó phòng chưa
    checkHasComment(){
        if ((this.truongPhongVal && this.tpComment != '') || (this.phoPhongVal && this.ppComment != '')) {
            return true;
        }
        return false;
    }
    //kiểm tra có bỏ trống đội chủ trì không
    checkHasDoiCT() {
        let checkDoiCT = this.data_department.find(x => x.mainHandling);
        if (checkDoiCT) {
            return true;
        }
        return false;
    ...
    save(): void {
        ...
        if (!this.checkHasComment()) {
            this.notify.error("Phải có Ý kiến chỉ đạo của Trưởng phòng hoặc Phó phòng.");
            return
        }

        if (!this.checkHasDoiCT()) {
            this.notify.error("Đội chủ trì không được bỏ trống.")
            return
        }
        ...
        // this._documentAppService.checkDaPhanCBCS(this.documentData.id).subscribe(res=>{
            ...
            // } else this.notify.warn("Văn bản đã phân xử lý cho cán bộ chiến sỹ, không thể chỉnh sửa!");
        // });
        // if(result.isValid){
        //     self.saving = true;
        //     self.documentData.isActive = true;
