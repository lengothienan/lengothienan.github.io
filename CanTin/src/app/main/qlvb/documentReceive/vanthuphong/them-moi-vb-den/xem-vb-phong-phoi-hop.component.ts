...
@Component({
    ...
    templateUrl: './xem-vb-phong-phoi-hop.component.html',
...
export class ViewDocumentDenPhongPhoiHopComponent extends AppComponentBase implements OnInit{
    ...
    ngOnInit(){
        ...
        const iddochandlingdetail = this._activatedRoute.snapshot.paramMap.get('iddochandlingdetail');
        ...
        if(Id != null){
            ...
            this._documentAppService.getDocumentForViewPhoiHop(this.iddochandlingdetail,this.documentId).subscribe((result) => {
                this.documentData = result;

                this.yKienBGD += this.documentData.approvingRecommended == null ? "" : this.documentData.approvingRecommended;
                this._documentHandlingAppService.get_DVXL_ForDocument_Dept_Phoi_Hop(this.iddochandlingdetail).subscribe((res) => {
                    this.old_DVXL = res;
                    console.log(res)
