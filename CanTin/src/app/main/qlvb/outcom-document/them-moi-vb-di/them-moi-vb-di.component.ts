...
import { DxFormComponent, DxDateBoxComponent, DxSelectBoxComponent, DxTagBoxComponent } from 'devextreme-angular';
...
@Component({
    selector: 'themMoiVBDiComponent',
    templateUrl: './them-moi-vb-di.component.html',
    styleUrls: ['./them-moi-vb-di.component.less'],
    encapsulation: ViewEncapsulation.None,
    providers: [DatePipe] ,
    animations: [appModuleAnimation()]

})
export class ThemMoiVBDiNewComponent extends AppComponentBase {
    ...
    chi_huy = [{userId: 1, nameWithPosition: 'Nguyễn Văn A'}, {userId: 2, nameWithPosition: 'Nguyễn Văn B'}];
    nguoi_ky = [];
    orgLevelSource = [];
    selectedOrgLevels = [];
    publishOrgSource = [];
    constructor(
        ...
        this.priority_value = this.priorities[0].id;
        this.bookVal = 1;
    }

    radio_nguoi_ky(){
        console.log(this.priority_value)
        if(this.priority_value == 1){
            this.nguoi_ky =  this.captain_department;
        }
        else if(this.priority_value == 2){
    ...
    back(){
        this.router.navigate(['/app/main/qlvb/danh-sach-vb-cho-phat-hanh']);
       // this._location.back();
    }
