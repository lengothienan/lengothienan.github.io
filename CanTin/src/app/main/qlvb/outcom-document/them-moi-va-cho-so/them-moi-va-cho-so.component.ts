...
import { DxFormComponent, DxLoadPanelComponent, DxDateBoxComponent, DxSelectBoxComponent, DxTextBoxComponent, DxTreeViewComponent, DxTagBoxComponent } from 'devextreme-angular';
...
@Component({
    selector: 'themMoiVaChoSo',
    templateUrl: './them-moi-va-cho-so.component.html',
    styleUrls: ['./them-moi-va-cho-so.component.less'],
    encapsulation: ViewEncapsulation.None,
    providers: [DatePipe],
    animations: [appModuleAnimation()]

})
export class ThemMoiVaChoSoComponent extends AppComponentBase {
    ...
    @ViewChild('loadPanel', { static: true }) loadPanel: DxLoadPanelComponent;
    ...
    header = "Thêm mới VB đi và cho số";
    isNotCATP = false;
    isNotEdited = false;
    selfOrg: number;
    ...
    constructor(
        ...
        this.checkSecretLevel = this.checkSecretLevel.bind(this);
        this.checkBookType = this.checkBookType.bind(this);
        ...
        this._dynamicFieldService.getDynamicFieldByModuleId(22, 0).subscribe(result => {
            ...
            this.secretLevelOptions = {
                dataSource: this.data_secretLevel,
                valueExpr: 'key',
                displayExpr: 'value',
                searchEnabled: true,
                searchExpr: 'value',
                showClearButton: true,
                value: this.secretVal,
                onValueChanged: this.onSecretLevelValueChanged }
        });
        this._oDocServiceProxy.getListOrgLevels().subscribe(res => {
            this.orgLevelSource = res;
        });

        // this.publishOrgSource = new DataSource({
        //     store: 
        // })

        // this.publishOrgSource = new CustomStore({
        //     key: "id",
        //     byKey: (key) => {
        //         let promise = new Promise((resolve, reject) => {
        //             this._oDocServiceProxy.getPublishOrgById(key).subscribe(res => {
        //                 resolve(res);
        //             }, err => {
        //                 reject(err);
        //             });
        ...
        //     load: (loadOptions) => {
        //         let promise = new Promise((resolve, reject) => {
        //             this._oDocServiceProxy.getListPublishOrgFilter(this.selectedOrgLevels).subscribe(res =>{
        //                 resolve(res);
        //             }, err => {
        //                 reject(err);
        //             });
    ...
    ngOnInit() {
        ...
        if (this.utility.selectedDocumentData != null && this.utility.selectedDocumentData != undefined) {
            ...
            let id = null;
            if (this.appSession.isCATP) id = this.selectedDocumentData.Id;
            else
            ...
            if (id > 0) {
                if (this.appSession.isCATP) {
                    this.selectedOrgLevels.push(this.selectedDocumentData.OrgLevelId);
            ...
            if (!isNullOrUndefined(this.selectedDocumentData.OrgEditor) || !isNullOrUndefined(this.selectedDocumentData.orgEditor)) {
                if (this.appSession.isCATP) this.documentData.orgEditor = this.selectedDocumentData.orgEditor;
                else
            ...
            this.utility.selectedDocumentData = null;
        }
        else {
            ...
            this.documentData.releaseDate = moment(new Date());
            console.log(this.documentData.releaseDate.toString())
            if (this.appSession.isCATP) {
...
/////////////////////////////////////////////////////
        if (Id != null && Id != undefined) {
            ...
            this._oDocServiceProxy.getReceiversByDocId(this.docId).subscribe(result => {
                ...
                this.displayReceiverList = that.arr;
                // this.documentData.receiverId = this.displayReceiverList;
            });
    ...
    async getODocListForEdit() {
        ...
        let promise = new Promise(function (resolve, reject) {
            self._oDocServiceProxy.getODocForEdit(self.docId).subscribe(res => {
                ...
                if(self.documentData.releaseDate==undefined || self.documentData.releaseDate==undefined==null){
                    self.documentData.releaseDate=moment()
                }
                ...
                self.quantityDocPublishVal = self.documentData.quantityDocumentPublish;
                self.piecesOfPaperVal = self.documentData.piecesOfPaper;
                self.secretLevelOptions = {
                    dataSource: self.data_secretLevel,
                    valueExpr: 'key',
                    displayExpr: 'value',
                    searchEnabled: true,
                    searchExpr: 'value',
                    showClearButton: true,
                    value: self.secretVal,
                    onValueChanged: self.onSecretLevelValueChanged }        
                if (res.attachment) {
    ...
    onContentReady = async (e) => {
        ...
        if (Id != null && Id != undefined) {
            ...
            if (isCATP == false) {
                ...
                //this.appSession.organizationUnitId == this.documentData.orgEditor || this.documentData.orgEditor == 2
                if (roleGroup == 'CATP') {
                    notCATP = false;
        ...
        this.bookOptions = {
            ...
            onValueChanged: async function (e) {
                ...
                if (self.currentSyntax.includes("{currentValue}") && self.numberChBkValue) {
                    this.getMissingOutcomingNumber(curBook.id);
                    self.currentNumberInBook = await this.getNextNumber(curBook.id);
    ...
    onComBookValueChanged = async (e) => {

        this.currentBookInMissingNumberPopup = this.dataBook.filter(x=> x.id == e.value)[0]
        this.numberChBkValue = true;
        if (this.numberChBkValue) {
            if (this.documentData.number == null) {
                let curBook = this.dataBook.find(x => x.id == parseInt(e.value));
                if (curBook["syntax"] == null) this.currentSyntax = "{currentValue}";
                else this.currentSyntax = curBook["syntax"];
                this.currentNumberInBook = this.currentSyntax;

                if (this.currentSyntax.includes("{currentValue}")) {
                    this.getMissingOutcomingNumber(curBook.id);
                    this.currentNumberInBook = await this.getNextNumber(curBook.id);
                ...
                this.documentData.number = this.documentData.number;
                if (e.event != null) {
                    let curBook = this.dataBook.find(x => x.id == parseInt(e.value));
                    if (curBook["syntax"] == null) this.currentSyntax = "{currentValue}";
                    else this.currentSyntax = curBook["syntax"];
    ...
    getMissingOutcomingNumber(soVB: number) {
        this.missingNumberSource = []
        this._oDocServiceProxy.getMissingOutcommingNumber(soVB).subscribe(res => {
            if (res.includes("ERROR")) {
                this.missingNumber = "Có lỗi trong quá trình xử lý, vui lòng liên hệ Admin để kiểm tra lại."
            }
            else if (res.length == 0) {
                this.missingNumber = "Không có số đi bị khuyết."
            }
            else {
    ...
    async getNextNumber(bookId: number): Promise<any> {
        let promise = new Promise((resolve, reject) => {
            this._oDocServiceProxy.getNextOutcommingNumber(bookId).subscribe(res => {
    ...
    numberChanged = async (e) => {
        const self = this;
        self.isNumberDisabled = !self.isNumberDisabled;
        self.numberChBkValue = e.value;
        self.isDraftNumberDisabled = e.value;
        if (e.value == true) { // nút tự động được chọn
            if (self.documentData.number == null) {
                let curBook = self.dataBook.find(x => x.id == self.documentData.book);
                if (curBook.syntax == null) self.currentSyntax = "{currentValue}";
                else
                    self.currentSyntax = curBook.syntax;
                if (self.currentSyntax.includes("{currentValue}")) {
                    ...
                    self.soKyHieu = await self.getNextNumber(curBook.id);
                }
                ...
                self.documentData.number = self.documentData.number;
    ...
    buttonSaveAndPublishClick() {
        if (!this.appSession.isCATP) {
            this.message.confirm(
                'Sau khi phát hành sẽ không thể chỉnh sửa văn bản. Vui lòng kiểm tra kĩ trước khi tiếp tục',
                'Chú ý!',
                isConfirmed => {
                    if (isConfirmed) {
                        this.documentData.isSaveAndPublish = true;
                        if (!this.documentData.number) {
                            this.save();
                        }
                        else {
                            this._oDocServiceProxy.checkNumberInBook(this.documentData.book, this.documentData.number).subscribe(res => {
                                if (res) {
    ...
    buttonSaveClick() {
        ...
        this.documentData.isSaveAndPublish = false;
    ...
    save(): void {
        ...
        if (result.isValid) {
            ...
            //nếu lưu và phát hành thì chuyển thẳng trạng thái đã xử lý
            if (this.documentData.isSaveAndPublish) {
                this.documentData.cA_Status = true;
            }
                ...
                this._oDocServiceProxy.themMoiVaChoSo(this.documentData)
                    ...
                    .subscribe(() => {
                        ...
                        if (this.documentData.isSaveAndPublish) {
                            ...
                            this.router.navigate(['/app/main/qlvb/danh-sach-vb-cua-dv-gui-dv-khac-chua-ph']);
                        }
    ...
    getClassName() {
        ...
        if (this.appSession.isCATP) return "them-moi-va-cho-so";
        return "";
    }
    ...
    checkSecretLevel(e: any) {
        var bookType: any;
        var isValid: any;
        if (this.documentData.book != undefined && this.documentData.book > 0) {
            for (let i = 0, len = this.dataBook.length; i < len; i++) {
                if (this.dataBook[i].id == this.documentData.book) {
                    bookType = this.dataBook[i].bookType;
                    break;
                }
            }
            if (bookType != undefined && e.value != undefined) {
                if (bookType == 1 && e.value == 0) {
                    isValid = true;
                } else if (bookType == 2 && e.value >= 1) {
                    isValid = true;
                } else {
                    isValid = false;
                }
    ...
    checkBookType(e: any) {
        ...
        if (this.documentData.book != undefined && this.documentData.book > 0) {
            ...
            if (bookType != undefined && this.documentData.secretLevel != undefined) {
                if (bookType == 1 && this.documentData.secretLevel == 0) {
                    isValid = true;
                } else if (bookType == 2 && this.documentData.secretLevel >= 1) {
                    isValid = true;
                } else {
                    isValid = false;
                }
    ...
    onSecretLevelValueChanged = async (e) => {
        if (this.documentData.secretLevel != undefined) {
            this.secretVal = this.documentData.secretLevel;
            this.documentForm.instance.validate();
        }
    }
}
