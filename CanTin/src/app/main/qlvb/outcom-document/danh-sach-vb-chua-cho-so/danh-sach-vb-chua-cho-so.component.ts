...
import { CachingServiceProxy, WebLogServiceProxy, AuditLogServiceProxy, HistoryUploadDto, HistoryUploadsServiceProxy, DocumentServiceProxy, Idoc_starsServiceProxy, CreateOrEditIdoc_starDto, DynamicActionsServiceProxy, DocumentHandlingDetailDto, DocumentHandlingsServiceProxy, DocumentsDto, DocumentHandlingDetailsServiceProxy, UserServiceProxy, DynamicGridHelperServiceProxy, DynamicFieldsServiceProxy, DirectorOpinionDto, ListDVXL, ApproveDocumentDto, DocumentTypesServiceProxy, DocumentTypeDto, ODocsServiceProxy, ODocStoreDto } from '@shared/service-proxies/service-proxies';
...
@Component({
    templateUrl: './danh-sach-vb-chua-cho-so.component.html',
    styleUrls: ['./danh-sach-vb-chua-cho-so.component.less'],
    animations: [appModuleAnimation()]
})
export class DanhSachVBChuaChoSoComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit() {
        ...
        if(this.appSession.roleId == 28){
            this._oDocsServiceProxy.getAllOutDocumentByOrgIdAndTypeHandling(this.appSession.organizationUnitId, 2, 1).subscribe(res => {
                this.initialData = res;

            });
        }
        else {
            this._oDocsServiceProxy.getAllOutDocumentByOrgIdAndTypeHandling(this.appSession.organizationUnitId, 2, 2).subscribe(res => {
                this.initialData = res;
    ...
    edit(e: any){
        this.router.navigate(['/app/main/qlvb/oDocs/edit/' + e]);
    }
