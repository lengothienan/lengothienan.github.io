...
import { CachingServiceProxy, WebLogServiceProxy, AuditLogServiceProxy, HistoryUploadDto, HistoryUploadsServiceProxy, DocumentServiceProxy, Idoc_starsServiceProxy, CreateOrEditIdoc_starDto, DynamicActionsServiceProxy, DocumentHandlingDetailDto, DocumentHandlingsServiceProxy, DocumentsDto, DocumentHandlingDetailsServiceProxy, UserServiceProxy, DynamicGridHelperServiceProxy, DynamicFieldsServiceProxy, DirectorOpinionDto, ListDVXL, ApproveDocumentDto, DocumentTypesServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    templateUrl: './danh-sach-vb-da-phe-duyet.component.html',
    styleUrls: ['./danh-sach-vb-da-phe-duyet.component.less'],
    animations: [appModuleAnimation()]
})
export class DanhSachVBDaPheDuyetComponent extends AppComponentBase implements OnInit {
    ...
    ngOnInit() {
        ...
        this._documentAppService.getDocumentVanThuPhong(this.appSession.organizationUnitId).subscribe(res => {
            ...
            for(var i = 0, len = this.initialData.length; i < len; i++){
                ...
                this.initialData[i].documentTypeName = this.initialData[i].documentTypeId != null ? this.documentTypeOptions.find(x => x.id == this.initialData[i].documentTypeId).typeName : "";
            }
        })
    }
    ...
    view(e: any){
        this.router.navigate(['/app/main/qlvb/them-moi-vb-di']);
