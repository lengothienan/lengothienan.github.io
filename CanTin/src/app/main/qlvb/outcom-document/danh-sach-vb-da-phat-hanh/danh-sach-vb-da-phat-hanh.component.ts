...
@Component({
    templateUrl: './danh-sach-vb-da-phat-hanh.component.html',
    styleUrls: ['./danh-sach-vb-da-phat-hanh.component.less'],
    animations: [appModuleAnimation()]
})
export class DanhSachVBDaPhatHanhComponent extends AppComponentBase implements OnInit {
    ...
    constructor(
        ...
        this._dynamicFieldService.getDynamicFieldByModuleId(22, 0).subscribe(result => {
            ...
            console.log(this.data_secretLevel = result[1].dataSource)
    ...
    search(){
        if(this.appSession.isCATP){
            this._oDocsServiceProxy.getListDocumentCATPDaPhatHanh(this.documentSearchData["book"] ==null?-1:this.documentSearchData["book"]).subscribe(res => {
                this.initialData = res;
            });
        }else {
            this._oDocsServiceProxy.getAllODocHadNumberByCBCS(this.appSession.userId).subscribe(res => {
                this.initialData = res;
                console.log(res)
    ...
    edit(e: any){
        this.router.navigate(['/app/main/qlvb/sua-vb-di/' + e]);
    }
    ...
    delete(e: any){
        ...
        // TODO
        const self = this;
        this._oDocsServiceProxy.deletePublishedDocument(e).subscribe(res => {
            self.search();
            self.gridContainer.instance.refresh();
            self.gridContainer.instance.repaint();
        });
    }
