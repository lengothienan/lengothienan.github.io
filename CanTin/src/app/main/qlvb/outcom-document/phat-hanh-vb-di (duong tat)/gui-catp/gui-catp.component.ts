...
@Component({
    selector: 'themMoiVBDiChuyenThayCATPComponent',
    templateUrl: './gui-catp.component.html',
...
export class ThemMoiVBDiChuyenThayCATPComponent extends AppComponentBase {
    ...
    @ViewChild('searchDocumentNoiBoModal', { static: true }) searchDocumentNoiBoModal: SearchDocumentNoiBoModalComponent;
    ...
    ngOnInit() {
        ...
        if (this._utilityService.selectedDocumentData !== undefined) {
            ...
            let arr = [];
            // Array.prototype.forEach.call(linkedDocument, child => {
            //     arr.push({ id: child.Id, incommingNumber: child.IncommingNumber });
            // });
            if(linkedDocument["Number"] != null){
                arr.push({ id: linkedDocument["Id"], incommingNumber: linkedDocument["Number"] });
            }
            else if(linkedDocument["IncommingNumberDV"] != null ) {
                arr.push({ id: linkedDocument["Id"], incommingNumber: linkedDocument["IncommingNumberDV"] });
            }
    ...
    save(): void {
        ...
        if (result.isValid) {
            ...
            this.documentData.publishType = 1;
            if (this.documentData.receiverId.length > 0){
                this.documentData.cA_Status = this.status;
                this.documentData.cA_Rating = 3; // hoàn thành nhiệm vụ
                this.documentData.cA_ProcessingRecommended = this.ProcessingResult;
            }
            console.log(this.documentData.linkedDocumentList)
            // this._oDocsServiceProxy.oDocTuDoiTransferDirectlyToCATP(this.documentData)
            // .pipe(finalize(() => {
            //     self.saving = false;
            ...
            // }))
            // .subscribe(() => {
            //     self.notify.info(this.l('Văn bản đã được chuyển cho Văn thư đơn vị'));
            //     // self.router.navigate(['/app/main/qlvb/danh-sach-vb-chua-phuc-dap-cua-doi-tu-catp']);
            // });
    ...
    selectReplyDocument() {
        this.searchDocumentNoiBoModal.show();
    }
            ...
            dataSource: {
                ...
                load: function () {
                    const promise = new Promise((resolve, reject) => {
                        that._commBookService.getAllODocBookOfCATP().subscribe(result => {
