...
@Component({
    selector: 'themMoiVBDiChuyenThayVTDVComponent',
    templateUrl: './gui-dv-khac.component.html',
...
export class ThemMoiVBDiChuyenThayVTDVComponent extends AppComponentBase {
    ...
    @ViewChild('searchDocumentDonViModal', { static: true }) searchDocumentDonViModal: SearchDocumentNoiBoModalComponent;
    ...
    ngOnInit() {
        ...
        if (this._utilityService.hasOwnProperty("selectedDocumentData") || this._utilityService.hasOwnProperty("documentData")) {
        ...
        // this.data_department.push({ id: this.appSession.organizationUnitId, displayName: this.appSession.orgName });
    ...
    orgLevelValueChanged(e: any) {
        ...
        for(let i = 0, len = e.value.length; i < len; i++){
            if(len === 1){
                break;
            }
            if(e.value[i] == 11) // 11: Ban Giám đốc
            {
                that.publishOrgTagBox.value = [];
                // let a = that.publishOrgTagBox.instance.option('value', [11]);
                that.publishOrgTagBox.value = [11];
                that.publishOrgTagBox.instance.repaint();
                that.selectedOrgLevels = [11];
                break;
            }
        }
        this.selectedOrgLevels.forEach(x => {
            Array.prototype.push.apply(that.publishOrgSource, that.data_receiver.filter(a => a.orgLevelId === x));
        });
    }
    ...
    save(): void {
        ...
        if (result.isValid) {
            ...
            this._oDocsServiceProxy.oDocSaveAndTransferToDonViDuongTat(this.documentData)
            .pipe(finalize(() => {
                self.saving = false;
                self.popup_publishType = false;
    ...
    saveReceiveList() {
        ...
        this.selectedReceivers.forEach(x => {
            ...
            if(x.itemData.org != null){
                item.org = x.itemData.org;
            }
            item.userId = x.itemData.userId
            arr.push(item);
        });
            ...
            dataSource: {
                ...
                load: function () {
                    const promise = new Promise((resolve, reject) => {
                        that._commBookService.getAllCommBookInDepartment("2", that.appSession.organizationUnitId).subscribe(result => {
                            that.documentData.book = result[0].id;
                            resolve(result);
