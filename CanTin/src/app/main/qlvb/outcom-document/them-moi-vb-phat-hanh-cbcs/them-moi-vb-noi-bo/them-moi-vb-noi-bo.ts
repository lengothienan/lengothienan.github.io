...
@Component({
    selector: 'themMoiVBNoiBoComponent',
    templateUrl: './them-moi-vb-noi-bo.html',
...
export class ThemMoiVanBanNoiBoComponent extends AppComponentBase {
    ...
    save(): void {
        ...
        if (result.isValid) {
            ...
            this._oDocsServiceProxy.createLocalODoc(this.documentData)
            .pipe(finalize(() => {
                this.saving = false;
                this.popup_publishType = false;
                this.router.navigate(['/app/main/qlvb/danh-sach-vb-chua-phuc-dap-cua-doi-tu-don-vi-khac']);
            }))
            .subscribe(() => {
    ...
    selectReplyDocument() {
        this.searchDocumentDonViModal.type = 3;
